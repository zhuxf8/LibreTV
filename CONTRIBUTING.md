...
- Node.js 18.0 或更高版本（服务端使用 Express 5）
- Git
- 支持 ES6 的现代浏览器

### 项目设计原则

- **无内置数据源**：本项目默认不包含任何视频源。所有采集站 / CMS API 均由用户在前端「设置」中添加，代码仓库内不维护任何默认源。
- **代理与服务端隔离**：前端只负责搜索/渲染；跨域抓取与 HLS 改写均在服务端函数（`server.mjs` / `api/` / `netlify/functions/` / `functions/`）完成。
- **安全第一**：代理鉴权、SSRF 拦截、源码保护为必保留能力，任何改动不得削弱这些防护。贡献涉及代理逻辑时请一并更新相关测试与文档。
