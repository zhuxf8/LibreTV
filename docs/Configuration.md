...
- **源订阅**：远程源列表地址与上次同步时间，格式与行为见 [Data-Sources · 源订阅](Data-Sources.md#源订阅--分享)。同步为手动操作（订阅条目 **⟳**），远端列表更新后需手动刷新。
...
| 方式 | 说明 |
| --- | --- |
| 内置代理（默认） | 经 `/api/proxy/` 转发（带豆瓣 Referer 伪装），规避豆瓣防盗链与图床直连失败 |
| 直连 | 浏览器直接加载豆瓣/采集站封面，最快；部分网络下豆瓣图会 418 |
