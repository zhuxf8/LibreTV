...
**分享（导出）**：设置 → 源订阅 / 分享 → 「导出源列表」，生成如下格式的 JSON 文件：
...
```json
{
  ...
  "version": 1,
  "sources": [
    { "name": "暴风资源", "url": "https://bfzyapi.com/api.php/provide/vod", "detail": "", "isAdult": false }
  ]
}
```

把文件托管到任意可公开访问的 URL（Gist、对象存储、自建静态服务均可），该 URL 就是一个「订阅地址」。也接受裸数组格式 `[{name,url,...}, ...]`。

**订阅（导入）**：设置 → 源订阅 / 分享 → 填入订阅地址 → 「订阅」：

- 服务端拉取并校验 JSON（协议白名单 + 内网地址过滤，单列表上限 100 个源，按 URL 去重）；
- 导入的源以 `sub_` 前缀 key 管理，自动勾选（成人过滤开启时跳过成人源）；
- 远端列表更新后点 **⟳** 重新同步（整体替换该订阅名下的源，与订阅 URL 相同的手动源会被去重合并）；
- 删除订阅会同时移除其导入的所有源。
